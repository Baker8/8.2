...
DISPLAYSURF = pygame.display.set_mode((resolution.current_w, resolution.current_h), pygame.NOFRAME)
            ...
            if note[1] * 60 < latest + (resolution.current_w - play_position) / GLOBALS.SpeedOMeter:
                ...
                if note[3] not in exclusionlist:
                    if int(note[2] * CLOCK * GLOBALS.SpeedOMeter) > 2000:
                        newnote = Note(2000 / GLOBALS.SpeedOMeter, note[4], (note[2] * GLOBALS.SpeedOMeter * CLOCK),
                                       note[3], note[0])
                    else:
                        newnote = Note(note[2] * CLOCK, note[4], (note[2] * GLOBALS.SpeedOMeter * CLOCK), note[3],
                                       note[0])
                    newnote.rect.y = window_height - int(
                        (note[0] - int(minscalar) + 1) * (window_height / (score_range + 1)))
        ...
        for player in player_list:
            ...
            for note in note_collisions:
                if player.rect.y + GLOBALS.player_height < note.rect.y + GLOBALS.notebuffer and player.vspeed > 0:
                    ...
                    if note.color == (255, 0, 0) and player.color == (0, 0, 255) or note.color == (
                    0, 0, 255) and player.color == (255, 0, 0):
